...
1. **The simplest bridge wins; added architectural complexity does not help.** Across both encoder pairs, a plain linear (or shallow MLP) bridge is as good as or better than anything more elaborate. For Audio-MAE/DINOv2, a single 768×768 linear map reaches **45.7% R@5** (chance: 2.0%) and beats the same-capacity nonlinear MLP (34.3%). The encoders — trained independently, on different modalities, with no shared supervision — align to a large degree under a simple affine transform.

2. **Neither encoder family is clearly superior; the small gap depends on the bridge.** With a linear bridge, the language-free pair leads (45.7% vs 37.8% R@5); with an MLP bridge, the text-mediated pair edges ahead (36.2% vs 34.3%). Both gaps are modest, and nothing resembles a decisive advantage either way — so "language-free aligns better" is not supported once the comparison is made symmetric.

3. **The proposed Fourier+SIREN bridge fails to generalize, and the failure is isolated to that block.** A ReLU MLP with the same depth, width, dropout, data, and training budget generalizes well (34.3% R@5); SpectralBridge, differing only in the frozen random Fourier projection and sine activations, performs at chance level (2.8% R@5). This is a clean ablation: the periodic-architecture hypothesis was tested and rejected on this task.

4. **Text-aligned encoders transfer poorly zero-shot but adapt cheaply.** Zero-shot CLAP→CLIP retrieval is at chance level (2.4% R@5); a small trained projection lifts it to 36–38%, competitive with the language-free pair.
...
Each encoder pair is tested with both a linear probe and a nonlinear bridge, so the encoder-vs-bridge effects can be separated.
...
- **Linear probe** (d→d): is a full-rank linear map sufficient? (no bottleneck, no nonlinearity). Run for both encoder pairs (768→768 and 512→512).
...
- **CLAP→CLIP trained MLP** (512→128→512): nonlinear bridge for the text-mediated pair.

This design tests two factors separately: the **encoder pair** (language-free vs text-mediated) and the **bridge** (linear vs nonlinear), with both pairs run under both bridge types.
...
- **Under-training.** With 2 gradient steps per epoch, an earlier budget (100 epochs, patience 15) stopped most bridges after ~30–40 steps. An apparent 7.6× gap between encoder pairs was largely an under-training artifact; with a proper budget the encoder pairs are roughly on par.
